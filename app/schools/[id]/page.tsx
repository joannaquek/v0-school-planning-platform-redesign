    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <main className="container mx-auto px-4 py-16 text-center">
          ...
          <p className="mt-2 text-muted-foreground">The school you&apos;re looking for doesn&apos;t exist.</p>
      ...
      <main className="container mx-auto px-4 py-6">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-2 flex items-center gap-2 text-muted-foreground">
                ...
                <span>{school.address}, Singapore {school.postalCode}</span>
              </div>
              {school.affiliation && (
            ...
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                <div className={cn(
                  ...
                  school.ballotChance && school.ballotChance >= 70 
                    ? 'bg-success/10 text-success' 
                    : school.ballotChance && school.ballotChance >= 40
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-8">
                ...
                <div className="grid gap-6 sm:grid-cols-2">
                  <div>
                    ...
                    <p className="mt-1 text-foreground">
                      {school.distanceBand === '1km' ? 'Within 1km (Priority)' : 
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-foreground">
                      {school.historicalData[0]?.balloted ? 'Yes (oversubscribed)' : 'No (places available)'}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">Data Phase</p>
                    <p className="mt-1 text-foreground">Phase 2C, 2025</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            {/* Historical Trends */}
            <TrendChart data={school.historicalData} schoolName={school.name} />
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-border">
                        <th className="pb-3 pr-4 text-left font-medium text-muted-foreground">Year</th>
                        <th className="pb-3 px-4 text-center font-medium text-muted-foreground">Phase</th>
                        <th className="pb-3 px-4 text-center font-medium text-muted-foreground">Vacancies</th>
                        <th className="pb-3 px-4 text-center font-medium text-muted-foreground">Registered</th>
                        <th className="pb-3 px-4 text-center font-medium text-muted-foreground">Balloted</th>
                        <th className="pb-3 pl-4 text-center font-medium text-muted-foreground">Success Rate</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-border">
                      {school.historicalData.map((data) => (
                        <tr key={data.year}>
                          <td className="py-3 pr-4 font-medium text-foreground">{data.year}</td>
                          <td className="py-3 px-4 text-center">{data.phase}</td>
                          <td className="py-3 px-4 text-center">{data.vacancies}</td>
                          <td className="py-3 px-4 text-center">{data.registered}</td>
                          <td className="py-3 px-4 text-center">
                            <Badge variant={data.balloted ? 'destructive' : 'secondary'} className="text-xs">
                              {data.balloted ? 'Yes' : 'No'}
                            </Badge>
                          </td>
                          <td className="py-3 pl-4 text-center font-medium">
                            {data.ballotRate ? `${data.ballotRate}%` : '100%'}
                          </td>
                        </tr>
                      ))}
                    </tbody>
